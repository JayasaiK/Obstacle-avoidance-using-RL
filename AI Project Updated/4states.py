...
def qtable(car_l, reward, move):
    ...
    '''a = gamma*(max(Q_table[row_index_new]))'''
...
while running:
    ...
    if epsilon>epsilon_min:
        ...
        finishl()
        pg.display.update() 
        #check if q learning is putting the car in a cycle of just two states, check if two moves are repeating and when it happens do randome choice..apparently its called soft choice something
